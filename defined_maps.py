...
def get_large_map1(tile_len, speed):
    ...
    spawn_points = Spawn_Points(tasks, map, [], max_tasks=max_cars,
                                out_file=f'tests/02/tmp_{str(solver.__class__.__name__)}_a{max_cars}_t{max_cars}')
...
def get_large_map2(tile_len, speed):
    ...
    cars_points = [Car_Point(map, cars, (2, 3), tile_len, 1, speed),
                   Car_Point(map, cars, (2, 11), tile_len, 1, speed),
                   Car_Point(map, cars, (8, 3), tile_len, 2, speed),
                   Car_Point(map, cars, (8, 11), tile_len, 2, speed),
                   Car_Point(map, cars, (14, 3), tile_len, 2, speed),
                   Car_Point(map, cars, (14, 11), tile_len, 2, speed)
                   ]
    ...
    spawn_points = Spawn_Points(tasks, map, [], max_tasks=max_cars,
                                out_file=f'tests/03/tmp_{str(solver.__class__.__name__)}_a{max_cars}_t{max_cars}')
    # static_points = Static_Points(tasks, map, [], in_file='inputs/Central_a8_t8.csv', out_file=f'tests/02/tmp_static_{str(solver.__class__.__name__)}_a{max_cars}_t{max_cars}')
